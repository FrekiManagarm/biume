...
export function WeekView({
  ...
  const getAppointmentsForDate = (date: Date) => {
    return appointments.filter((appointment) => {
      ...
      const appointmentDate = new Date(appointment.beginAt);
      return (
  ...
  const getAppointmentPosition = (appointment: Appointment) => {
    ...
    // Le calendrier commence à 8h, donc on soustrait 8h (480 minutes)
    const minutesFromStart = minutesFromMidnight - 8 * 60;
  ...
  const handleAppointmentClick = (appointment: Appointment) => {
    ...
    const relatedAppointments = appointments.filter(
      (apt) => apt.beginAt === appointment.beginAt,
    );
    setSelectedAppointments(relatedAppointments[0]);
