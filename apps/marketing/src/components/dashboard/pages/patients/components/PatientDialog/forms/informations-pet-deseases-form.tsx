...
const InformationsPetDeseasesForm = ({
  ...
  const [selectedDeseases, setSelectedDeseases] = useState<string[]>(
    (initialPetData?.deseases as string[]) || [],
  );

  const { data: fetchedPetData, isLoading } = useQuery({
    queryKey: ["pet", petId],
    queryFn: () => getPatientById(petId as string),
    enabled: !!petId && isUpdate && !initialPetData,
  });

  const petData =
    initialPetData ||
    (fetchedPetData && "data" in fetchedPetData ? fetchedPetData.data : null);
  ...
  const { mutateAsync } = useMutation({
    mutationFn: (deseases: string[]) =>
      updatePatientDeseases(deseases, petId as string),
    onSuccess: () => {
  ...
  const handleSubmit = async () => {
    ...
    await mutateAsync(selectedDeseases);
  };
